...
+ kube-node1：10.211.55.27
+ kube-node2：10.211.55.28
+ kube-node3：10.211.55.29
...
``` bash
cat  >>/etc/hosts <<EOF
10.211.55.27 kube-node1 kube-node1
10.211.55.28 kube-node2 kube-node2 
10.211.55.29 kube-node3 kube-node3
EOF
```
...
``` bash
sudo useradd -m k8s
sudo sh -c 'echo zkyd1225 | passwd k8s --stdin' # 为 k8s 账户设置密码
sudo visudo
