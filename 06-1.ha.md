...
listen kube-master
    ...
    option tcplog
    balance source
    server 10.211.55.27 10.211.55.27:6443 check inter 2000 fall 2 rise 2 weight 1
    server 10.211.55.28 10.211.55.28:6443 check inter 2000 fall 2 rise 2 weight 1
    server 10.211.55.29 10.211.55.29:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ master: 10.211.55.27
+ backup：10.211.55.28、10.211.55.29
...
``` bash
scp keepalived-master.conf root@10.211.55.27:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

``` bash
scp keepalived-backup.conf root@10.211.55.28:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@10.211.55.29:/etc/keepalived/keepalived.conf
```
...
http://10.211.55.252:10080/status
